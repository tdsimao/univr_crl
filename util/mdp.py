    ...
    for s, state_transitions in env.P.items():
        for a, state_action_transitions in state_transitions.items():
            for tr in state_action_transitions:
                ...
                if done:
                    terminal[ns] = True
    return transition, reward, cost, terminal, env.isd
...
def policy_evaluation(env, agent, horizon, discount_factor=1, verbose=False):
    transition, reward, cost, terminal, isd = get_mdp_functions(env)
    ns, na = reward.shape
    v_r = np.zeros(ns)
    v_c = np.zeros(ns)
    episode_length = np.zeros(ns)  # episode length
    policy = agent.get_policy()

    with trange(horizon, desc="policy evaluation", unit='steps', disable=not verbose) as progress:
        for i in progress:
            t = horizon - i - 1
            v_r[terminal] = 0
            v_c[terminal] = 0
            episode_length[terminal] = 0
            v_r = (policy[t] * (reward + (discount_factor * transition.dot(v_r)))).sum(axis=1)
            v_c = (policy[t] * (cost + (discount_factor * transition.dot(v_c)))).sum(axis=1)
            episode_length = (policy[t] * (1 + transition.dot(episode_length))).sum(axis=1)
    return (v_r * isd).sum(), (v_c * isd).sum(), (episode_length * isd).sum(), 0
