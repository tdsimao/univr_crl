...
from util.mdp import policy_evaluation


def train_agent(agent, env, number_of_episodes, horizon, seed, out_dir=None, discount_factor=1,
                label='', position=0, verbose=True):
    env.seed(seed)
    env.reset()
    agent.seed(seed)
    np.random.seed(seed)
    results = run_training_episodes(agent, env, number_of_episodes, horizon, discount_factor,
...
def run_training_episodes(agent, env, number_of_episodes, horizon, discount_factor=1, label='', position=0,
    ...
    with trange(number_of_episodes, desc=desc, unit="episode", position=position, disable=not verbose) as progress_bar:
        for i in progress_bar:
            ...
            evaluation = policy_evaluation(env, agent, agent.horizon, discount_factor)
...
def run_experiment(env_id, env_kwargs, agent_name, agentClass, agent_kwargs, seed,
                   number_of_episodes, position, out_dir):
    ...
    train_agent(agent, env, number_of_episodes, agent.horizon, seed, out_dir=agent_out_dir,
                label=agent_name, position=position)


def run_experiments_batch(agents, env_id, env_kwargs, number_of_episodes, out_dir, seeds, parallel=True):
    experiments = []
    for seed in seeds:
